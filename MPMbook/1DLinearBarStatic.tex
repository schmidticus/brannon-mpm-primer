...
\end{equation}
The superscript ``T'' denotes the transpose, which converts a column array to a row array, reversing the matrix dimensions as indicated.  Multiplying these two arrays is the equivalent of a dot or inner product, and results in a 1\by1 matrix, which is simply a single number.  Evaluating the above expression for $w(x)$ at the endpoints of the bar, note that the boundary term in the weak form may be written
\begin{equation}
